...
jobberdf = jobber[['Part Number','Make','Model','Start Year','End Year','Aces Part Type Desc','Submodel']]
ymmsr = pd.read_excel("Z:\Hao\jobace\AcePartType.xlsx", sheet_name=1, header=0)
ymmsrVar = ymmsr[ymmsr["Region"] == "United States"][['YearID','Make','Model','SubModel']]
...
for part_number, make, model in zip(part_numbers, makes, models):
    ...
    if not matches.empty:
        results.append(matches[['Part Number', 'Start Year', 'End Year', 'Make', 'Model', 'Aces Part Type Desc']])
    else:
        ...
        results.append(pd.DataFrame({
            ...
            'Make': [make],
            'Model': [model],
            'Aces Part Type Desc': [None]
        }))

# Concatenate all results into a single DataFrame
result_df = pd.concat(results, ignore_index=True)
result_df['Aces Part Type Desc'] = result_df['Aces Part Type Desc'].replace('Truck Cab Protector / Headache Rack', 'Truck Cab Protector - Headache Rack')
#print(result_df[result_df['Aces Part Type Desc']=='Truck Cab Protector - Headache Rack'])
...
expanded_result_df = expand_year_range(result_df).reset_index(drop=True)
...
final_unique = final_result_df.drop_duplicates().reset_index(drop=True)
final_unique.rename(columns = {'Part Number':'part',
                               'Aces Part Type Desc':'partterminologyname',
                               'Make':'make','Model':'model',
                               'Year':'year',
                               'SubModel':'submodel'
                               }, inplace=True)

final_unique['sku'] = final_unique['part']
final_unique['brand_code'] = 'GHTW'
final_unique['notes'] = ''
final_unique['qty'] = ''
